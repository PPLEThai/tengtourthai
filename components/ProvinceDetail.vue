<template>
  ...
  <div v-if="province" class="mt-4 border border-gray-200 rounded-lg p-4 shadow-sm overflow-y-auto">
    <div v-if="province.visits > 0" class="flex flex-row gap-2">
      <img src="@/assets/images/teng-detail.png" alt="" class="w-10 h-10" />
      <div class="flex flex-col">
        <h2 class="text-xl font-bold mb-2">{{ province.name }}</h2>
        ...
        <div v-if="province.actions && province.actions.length" class="flex space-x-2 mt-2">
          <span v-for="action in province.actions" :key="action" class="badge bg-green-300 text-sm font-light">{{ action
          }}</span>
        </div>
      </div>
        ...
        <Modal v-if="showModal" @close="showModal = false">
          ...
          <template v-slot:body>
            <div v-for="(post, index) in province.posts" :key="index" class="mb-2">
              <a :href="post" target="_blank" class="text-blue-500 underline">{{
                post
              }}</a>
            </div>
          </template>
    ...
    <div v-for="(details, name) in allProvinces" :key="name"
      ...
      <div class="flex flex-row gap-2">
        ...
        <div class="flex flex-col">
          ...
          <div v-if="details.visits > 0" class="flex flex-row gap-1">
            <p class="text-sm text-gray-500 font-light">
          ...
          <div v-if="details.actions && details.actions.length" class="flex space-x-2 mt-2">
            <span v-for="action in details.actions" :key="action" class="badge bg-green-300 text-sm font-light">{{
              action
            }}</span>
          </div>
        </div>
