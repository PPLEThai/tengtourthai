export interface MPImageMapping {
  thaiName: string;
  englishName: string;
}

export const mpImageMappings: MPImageMapping[] = [
  ...
  { thaiName: "ธัญธร_ธนินวัฒนาธร", englishName: "thanyathon_thaninwattanathorn" },
  ...
  { thaiName: "ปารมี_ไวจงเจริญ", englishName: "paramee_wijoncharoen" },
];

export function getEnglishImageName(firstname: string, lastname: string): string {
  const thaiName = `${firstname}_${lastname}`;
  const mapping = mpImageMappings.find(m => m.thaiName === thaiName);
  return mapping ? mapping.englishName : thaiName;
}

export function getThaiImageName(englishName: string): string {
  const mapping = mpImageMappings.find(m => m.englishName === englishName);
  return mapping ? mapping.thaiName : englishName;
} 