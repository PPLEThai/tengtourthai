  ...
  <div class="min-h-screen bg-[#0A2940] flex flex-col">
    ...
    <div v-else-if="mp"
      ...
      <main class="w-full md:flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        ...
        <!-- Social Media Embeds -->
        <div class="bg-white rounded-2xl p-4 md:p-6 h-[250px] md:h-[400px] col-span-1 md:col-span-2 flex flex-col">
          <h3 class="text-[#FF6A13] font-bold text-lg md:text-2xl mb-2">โซเชียลมีเดีย</h3>

          <!-- แสดง embedded social media ตามที่มีข้อมูล -->
          <div v-if="mp" class="flex-1 overflow-y-auto custom-scrollbar">
            <!-- Facebook -->
            <div v-if="mp.fb" class="mb-4">
              <h4 class="text-[#0A2940] font-semibold text-sm mb-2 flex items-center gap-2">
              ...
              <div v-if="mp.fb" class="bg-gray-50 rounded-lg p-3 border">
                ...
                <!-- Loading state -->
                <div v-if="facebookLoading" class="mt-3 text-center py-8">
                  ...
                  <p class="text-xs text-gray-400 mt-1">URL: {{ facebookPageUrl || 'กำลังประมวลผล...' }}</p>
                </div>
                
                <!-- Error state -->
                <div v-else-if="facebookError" class="mt-3 text-center py-4">
                  <p class="text-xs text-red-500 mb-2">{{ facebookError }}</p>
                  <p class="text-xs text-gray-400 mb-2">Original URL: {{ mp.fb }}</p>
                  <p class="text-xs text-gray-400 mb-2">Parsed URL: {{ facebookPageUrl }}</p>
                  <a :href="mp.fb" target="_blank" rel="noopener noreferrer"
                    class="text-[#1877F2] text-xs hover:underline">
                ...
                <div v-else-if="facebookPageUrl" class="mt-3">
                  <p class="text-xs text-gray-400 mb-2">Facebook page: {{ facebookPageUrl }}</p>
                  <div 
                    class="fb-page" 
                ...
                <!-- Fallback -->
                <div v-else class="mt-3 text-center py-4">
                  <p class="text-xs text-gray-600">
                    ติดตามข่าวสารและกิจกรรมล่าสุดของ {{ mp.fullname }}
                  </p>
                  <p class="text-xs text-gray-400 mt-1">Original URL: {{ mp.fb }}</p>
                  <p class="text-xs text-gray-400">Parsed URL: {{ facebookPageUrl }}</p>
                </div>
              </div>
              
              <!-- Fallback for non-Facebook URLs -->
              <div v-else class="bg-gray-50 rounded-lg p-3 border">
                <div class="flex items-center justify-between mb-2">
                  <a :href="mp.fb" target="_blank" rel="noopener noreferrer"
                    class="text-[#1877F2] text-sm hover:underline font-medium">
                    {{ getSocialMediaDisplayName(mp.fb) }}
            ...
            <div v-if="mp.ig" class="mb-4">
              ...
              <div class="bg-gray-50 rounded-lg p-3 border">
                ...
                <div class="text-xs text-gray-600">
                  ดูภาพและสตอรี่ล่าสุดของ {{ mp.fullname }}
                </div>
              </div>
            </div>

            <!-- X (Twitter) -->
            <div v-if="mp.x" class="mb-4">
              <h4 class="text-[#0A2940] font-semibold text-sm mb-2 flex items-center gap-2">
                <svg class="w-4 h-4 text-[#000000]" fill="currentColor" viewBox="0 0 24 24">
                  <path
                    d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
                X (Twitter)
              ...
              <div class="bg-gray-50 rounded-lg p-3 border">
                ...
                <div class="text-xs text-gray-600">
                  ติดตามความคิดเห็นและข่าวสารล่าสุดของ {{ mp.fullname }}
                </div>
              </div>
            </div>

            <!-- TikTok -->
            <div v-if="mp.tiktok" class="mb-4">
              <h4 class="text-[#0A2940] font-semibold text-sm mb-2 flex items-center gap-2">
                <svg class="w-4 h-4 text-[#000000]" fill="currentColor" viewBox="0 0 24 24">
                  <path
                    d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
              ...
              <div class="bg-gray-50 rounded-lg p-3 border">
                ...
                <div class="text-xs text-gray-600">
                  ดูวิดีโอและเนื้อหาสั้นๆ ของ {{ mp.fullname }}
                </div>
              </div>
            </div>

            <!-- กรณีไม่มี social media -->
            <div v-if="!mp.fb && !mp.ig && !mp.x && !mp.tiktok" class="text-center py-8">
              ...
              <p class="text-[#0A2940] text-sm">ยังไม่มีข้อมูลโซเชียลมีเดีย</p>
            </div>
          </div>
        </div>
